...
- [ ] `GITHUB_REMOTE` 값이 실제 ArgoCD repoURL과 일치하는가
- [ ] 현재 Git 원격이 public인지 private인지, ArgoCD repository secret이 필요한가
- [ ] AppProject를 `platform-operators`, `rhoai-core`, `rhoai-poc`로 나눌지 또는 더 단순화할지
